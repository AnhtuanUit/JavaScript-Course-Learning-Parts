...
function printBookAuthorsCount(bookName, ...authors) {
  console.log(`The book "${bookName}" has ${authors.length} authors`);
}
...
for (const { author = [] } of books) {
  if (typeof author === 'string') {
    ...
    for (const el of author) allAuthors.push(el);
  }
}
...
// console.log(quote.slice('boxing'.length * -1));
...
function isContributor(str) {
  return str.endsWith('(Contributor)');
}
...
function normalizeAuthorName(author) {
  ...
  const [firstName, lastName] = normalizedAuthor.split(' ');
  const capitalizedStr = str => str.replace(str[0], str[0].toUpperCase());
